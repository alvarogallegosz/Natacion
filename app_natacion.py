...
def login_usuario(user, password):
    try:
        # Verificación segura mediante Hash SHA-256 de la contraseña introducida
        ...
        # Compatibilidad hacia atrás: si no encuentra el hash, verifica texto plano y lo migra
        if not response.data:
            ...
            if response_plano.data:
                ...
                # Actualiza a hash silenciosamente en la base de datos
                    ...
                    if nuevo_nombre and nuevo_usuario and nueva_contrasena and nuevo_email:
                        try:
                            ...
                            if chequeo.data:
                                ...
                                # Las contraseñas se guardan inmediatamente con hash seguro
                        ...
                        try:
                            ...
                            if verificacion.data:
                                ...
                                if user_info.get("estatus") in ["Suspendido", "Bloqueado"]:
                                    ...
                                    # Actualización de contraseña aplicando hash
...
# -------------------------------------------------------------
# CARGA DE DATOS HISTÓRICOS (Corregido el error con ordenamiento .order("edad"))
# -------------------------------------------------------------
try:
    response = supabase.table("marcas_historicas") \
        ...
        .order("edad", desc=False).execute()  # ¡Ordenado de manera ascendente por la columna 'edad'!
...
# -------------------------------------------------------------
# MÓDULOS DE GESTIÓN SEGÚN ROL (Formularios y Tablas mejoradas)
# -------------------------------------------------------------
st.markdown("---")
    ...
    with col_vistas:
        ...
        if len(df_procesado) > 0:
            ...
            if st.session_state.rol in ["Entrenador", "Administrador"]:
                ...
                # Dropdown con descripción legible ocultando el ID crudo
