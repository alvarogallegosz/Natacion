...
if simulacion_externa:
    ...
    # 1. Definir los nombres de todas las pestañas posibles en el sistema
    diccionario_pestañas = {
        "📝 Pizarra Diaria": "tab_pizarra",
        "📊 Reportes de Entrenamiento": "tab_reportes",
        "📋 Resultados de competencias": "tab_marcas",
        "⏱️ Marcas Mínimas": "tab_config_marcas",
        "🎯 Asignaciones de Nadadores": "tab_asignaciones",
        "📅 Calendario Anual de Competencias": "tab_calendario",
        "🛡️ Consola Global (Admin)": "tab_admin"
    }

    # 2. Filtrar las pestañas permitidas según el rol actual del usuario
    rol_actual = st.session_state.get("rol", "Nadador")

    if rol_actual == "Nadador" or rol_actual == "Entrenador":
        pestañas_permitidas = [
            "📝 Pizarra Diaria", 
        ...
        pestañas_permitidas = list(diccionario_pestañas.keys())
    else:
        # Por seguridad, rol desconocido ve lo mismo que Nadador
        pestañas_permitidas = [
            "📝 Pizarra Diaria", 
            "📊 Reportes de Entrenamiento", 
            "📋 Resultados de competencias", 
    ...
    # 3. Crear las pestañas dinámicamente en Streamlit
    lista_tabs_renderizados = st.tabs(pestañas_permitidas)
    
    # 4. Mapear dinámicamente los contenedores a variables para mantener compatibilidad
    # Si una pestaña no está permitida para el rol, su variable será asignada como un contenedor vacío o None
    tab_pizarra = None
    tab_reportes = None
    tab_marcas = None
    tab_config_marcas = None
    tab_asignaciones = None
    tab_calendario = None
    tab_admin = None

    for titulo, obj_tab in zip(pestañas_permitidas, lista_tabs_renderizados):
        variable_destino = diccionario_pestañas[titulo]
        globals()[variable_destino] = obj_tab

    # 5. Inyección de contenido estructurado en cada pestaña activa
    for titulo, contenedor in zip(pestañas_permitidas, lista_tabs_renderizados):
        with contenedor:
            if titulo == "📝 Pizarra Diaria":
                st.markdown("### 📝 Pizarra Diaria")
                with tab_marcas:
